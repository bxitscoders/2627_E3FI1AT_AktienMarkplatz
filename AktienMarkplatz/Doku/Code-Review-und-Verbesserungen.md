...
|---|---|---|
| **S** – Single Responsibility (eine Aufgabe pro Klasse) | 🟡 teilweise | Gut: `FinnhubVerbindung` macht nur Aktien, `MassiveVerbindung` nur News, `Position` nur eine Depot-Position. Schlecht: `Aktie` liefert CSS-Klassen (`VeraenderungCssKlasse`), die Verrechnungskonto-Seite rechnet die Gebühr selbst aus, obwohl das schon in `Konto` steht. |
...
| **D** – Dependency Inversion (von Abstraktionen abhängen) | ❌ nein | Die Seiten erzeugen `new FinnhubVerbindung(...)` selbst, `KontoManager` ist statisch. Das Briefing nennt DIP aber ausdrücklich („SOLID, v. a. DIP“). |
...
| **Singleton** (im Ansatz) | statischer `HttpClient` in `ApiVerbindung`, statische Klasse `KontoManager` |
...
1. **Die Seiten sind nicht geschützt.** Nirgends steht `[Authorize]`. Wer `/Aktienmarkt` oder `/Verrechnungskonto` direkt aufruft, kommt ohne Login rein. Der Login ist damit nur eine Umleitung auf der Startseite.
...
3. **Vorgaben aus dem Briefing fehlen:**
   ...
   - keine Tests. „Automatisch testbar“ und xUnit-Tests sind vorgegeben. `Aktie`, `Position` und `Konto` wären leicht zu testen.
...
7. **Zwei Klassen in `Konto.cs`:** `KontoManager` sollte eine eigene Datei bekommen. Außerdem sind die Namen gemischt, z. B. `TransferTo` und `OwnerId` neben `Einzahlen`.
8. **Kontostände liegen nur im Speicher** und sind nach jedem Neustart weg.
9. **Geldbeträge sind uneinheitlich:** Kurse sind `double`, Kontostände `decimal`. Für Geld sollte man durchgehend `decimal` nehmen.
10. **`Einstellungen` hat kleingeschriebene Properties** (`newEmail`, `newPasswort`) und Englisch statt Deutsch.
...
4. Kleine Fehler beheben (Punkte 4–7, 10).
5. Tests für `Aktie`, `Position` und `Konto` schreiben.
